...
- **Ask box** plus a **live agent activity stream** (the autonomy money-shot) plus cited prospect cards (built with the `frontend-design` skill).
- `lib/runStream.ts`: `runAgent()` consumes the `/run` SSE stream from `../server/` and **falls back to `runMockStream()`** (driven by `sample_prospects.json`) if the server is down or `EventSource` is unavailable. The UI never branches, it's the same step/param/result shape either way.
