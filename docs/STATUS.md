...
3. **✅ READY [coord] Auto-match agent — optional ~25s 2nd clip.** Record with one command: **`python server/auto_match.py demo`** (full-screen terminal). Bulletproof: real ClickHouse data, NO Gemini (params hardcoded) so it can't flake. Story: define 2 campaigns in plain English → agent auto-fills them with real donors (Steyer etc.) → add a 3rd campaign live → it auto-matches it AND shows the existing two as "0 new" (proves continuous dedup) → "30 real donors auto-matched, none hand-picked; re-runs every 5 min." Only record if the main `?demo=1` take is done with time to spare — don't delay the main recording.
