...
Every run streams to the UI as Server-Sent Events, so judges *watch* the agent parse → query → enrich → score in real time.

## How we built it
- **ClickHouse Cloud** — the engine. We loaded **2.8M real FEC individual contributions** (official FEC API), tagged committees to ~20 causes, and query it in milliseconds. A dedup-safe query layer (`query_clean.py`) guarantees honest donor totals even with messy source rows.
- **Nimble** — live web enrichment. For each top donor we run a Nimble web search and distill it (employer, role, philanthropy, board seats) into a clean profile, cached for instant display.
- **Gemini** (`gemini-flash-latest`) — turns the free-text ask into structured query params; the autonomy "type any sentence" magic.
- **FastAPI + Server-Sent Events** — a `/run` endpoint streams each agent step to the UI live.
- **React + Vite + TypeScript + Tailwind** — a "donor-intelligence terminal": ask box, live activity stream, cited prospect cards with `fec.gov ↗` source pills.
- **Data source: the official FEC API** — real public records, the same ones our citations point to. No scraping, no synthetic data.

## Sponsor tools used
- **ClickHouse** — real-time analytical queries over 2.8M+ FEC rows; the core of the product.
- **Nimble** — live, real-time web enrichment of donor candidates; the "acts on the open web" autonomy step.
...
- **API rate limits** — Gemini's free tier 429'd under load; moved to Tier 1.
...
- The **continuous auto-match agent** — real autonomy: campaigns that fill themselves.

## What we learned
- Political giving is a surprisingly strong proxy for charitable cause-affinity — and the framing matters: this is **research on public giving behavior**, not a solicitation list (FEC's 11 CFR §104.15 restricts using contributor data to *solicit*; we cite the public record and never repackage it as a contact-to-ask list).
- For "sentence → known categories," an LLM classifier beats embeddings/RAG — cheaper, faster, no vector infra.
