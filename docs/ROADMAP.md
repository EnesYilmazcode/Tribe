...
- **FEC has none.** The individual-contributions file is name / city / state / zip /
  employer / occupation / amount / date — no email, no phone. So contact info cannot
  come from the database; it must be appended.
- **Sources to add later:**
  - Nimble enrichment — sometimes surfaces a public work email / org contact / LinkedIn
...
- Draft a personalized ask per prospect (cause + giving history → tailored email), then
  send / queue via the user's email. Human-in-the-loop approve before send.
