...
- A **sponsor tool is a sponsor company/platform**, not a product inside it.
- Rule: **use 2+ sponsor tools.** Judging rewards **3+** (Tool Use is 20% of score).
- Using multiple products from ONE sponsor (e.g. Nimble Search plus Crawl) still counts as **one** tool for the threshold, but deepens the "effective use" score.
...
- **x402 is NOT a prize-track sponsor.** It's only listed as a suggested payment rail. **We are not building it** (an onchain wallet plus facilitator is too much risk for a 5-hour sprint with no track payoff). Documented at the bottom for reference only.
...
**Our use:** Load FEC bulk individual-contribution data (~tens of millions of rows). Query by cause tag, geography, and capacity band to instantly rank candidate donors by giving behavior.
**How to access:**
...
- **Search API**, AI agents search the live web for precise info plus optional answer synthesis. `nimble.search(query=..., include_answer=True)`. Best for "what is this person doing now."
...
**What it is:** "The context layer for AI agents" (YC W24). CMS for the agentic web that ingests knowledge, searches it, generates content, and **publishes citeable content to cited.md**.
...
- Workflow: ingest source data, generate a grounded draft, then publish citeable to cited.md.
**Track (READ CAREFULLY):** $3k credits, **1 winner**. Must **close the loop from knowledge base to published, agent-discoverable content.** Ingestion alone does NOT qualify, you must publish a cited.md (or other public) destination. This is the highest-EV track (smallest field, cleanest fit).
...
**How it works:** the agent requests a paid resource, the server returns HTTP 402 with payment instructions, the agent signs a stablecoin authorization, attaches proof and retries, and the server verifies and returns data. A **facilitator** (Coinbase/CDP hosts one) abstracts the blockchain via `POST /verify` and `POST /settle`.
**Free tier:** CDP facilitator processes ERC-20 payments on Base, Polygon, Arbitrum, World, and Solana, with **1,000 transactions/month free.**
...
1. **ClickHouse**, the spine, do first. Covers Autonomy and the ClickHouse track.
...
- **FEC bulk load:** the full individual-contributions file is huge, so load a state/year-trimmed subset for the demo.
